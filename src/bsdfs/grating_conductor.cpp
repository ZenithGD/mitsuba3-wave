...
.. _bsdf-grating-conductor:

Smooth diffraction grating conductor (:monosp:`conductor`)
-------------------------------------------
...
public:
    ...
    GratingConductor(const Properties &props) : Base(props) {
        ...
        // initialize grating
        m_grating_angle = props.texture<Texture>("grating_angle", .0f);
        m_lobes = props.get<uint32_t>("lobes", 0U);
        bool radial = props.get<bool>("radial", false);

        // get grating type
        if (props.has_property("type")) {
            std::string gtype = string::to_lower(props.string("type"));
            if (gtype == "sinusoidal")
                m_grating_type = +DiffractionGratingType::Sinusoidal;
            else if (gtype == "rectangular")
                m_grating_type = +DiffractionGratingType::Rectangular;
            else if (gtype == "linear")
                m_grating_type = +DiffractionGratingType::Linear;
            else
                Throw("Specified an invalid distribution \"%s\", must be "
                      "\"sinusoidal\", \"rectangular\" or \"linear\"!", gtype.c_str());
        } else {
            m_grating_type = +DiffractionGratingType::Rectangular;
        }

        // check if grating is radial
        if ( radial ) {
            m_grating_type = m_grating_type | DiffractionGratingType::Radial;
        }
    }
    ...
    std::string to_string() const override {
        ...
        oss << "SmoothGratingConductor[" << std::endl
...
private:
    ...
    ref<Texture> m_inv_period;
    ref<Texture> m_height_scale;

    uint32_t m_grating_type;
    uint32_t m_lobes;
    Float m_multiplier;
};
